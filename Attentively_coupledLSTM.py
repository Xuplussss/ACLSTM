...
SAVE_DATE = '0715'
SAVE_NAME = SAVE_DATE + 'AFEW_allmean_NTNweight'
SAVE_FOLD = False
USE_CUDA = True
USE_pickle = False
# TG_PATH = 'data/auto_segment_tg/'
# ====data========================================
VIDEO_TAG_PATH = './data/Relabel/all_av_tag/'
VIDEO_PATH = './data/AV/'
WAV_PATH = './data/BAUM_audio_mono_16k/'
# WAV_PATH = './data/ffmpeg/'
# ====small data==================================
# VIDEO_TAG_PATH = './data/small_AVtag/'
# VIDEO_PATH = './data/small_AV/'
# WAV_PATH = './data/small_wav/'
# ================================================
MODEL_PATH = './model/'
RESULT_PATH = './result/'
# sound_model = '06284sound_pool2_filter100_total.pt'
# emotion_model = '06287emotion_pool2_filter200_total.pt'
# sound_model = '07164sound_pool2_filter50_total.pt'
# emotion_model = '0716_7emo_pool10_filter300_total.pt'

# ==== old model =========================================================
sound_model = '07294sound_pool2_filter50_total.pt'
...
NTN_model = '0616_cntn_T3_E50_B64_L0.0001.pkl'
VGG_model = 'BAUM_img_7emo.t7'
...
SND_CNN_FILTER, SND_CNN_POOL = 100, 2
EMO_CNN_FILTER, EMO_CNN_POOL = 100, 1
LR = 0.01
...
if USE_pickle:
	f = open('./feature/New_tag0616visual_seqs_BAUM_img_7emo.pkl', 'rb')
	visual_seqs = pickle.load(f)
	f.close()
	f = open('./feature/New_tag0616speech_emotion_seqs_0616Conv_K256_7emo_pool1_filter100_total.pkl', 'rb')
	speech_emotion_seqs = pickle.load(f)
	f.close()
	f = open('./feature/New_tag0616seqs_0616Conv_K256_7emo_pool1_filter100_total.pkl', 'rb')
	seqs = pickle.load(f)
	f.close()
	f = open('./feature/New_tag0616img_feature_BAUM_img_7emo.pkl', 'rb')
	...
	for video in sorted(os.listdir(VIDEO_PATH)):
		if video.endswith('mp4'):
			...
			for segment_index in range(len(boundary)-1):
				...
				num_of_frame = 0
				failed = 0
				for seg_frame in range(int((boundary[segment_index + 1] - boundary[segment_index])* (frame_count / boundary[-1]))): 			
					try:
						cap_frame_index = int((boundary[segment_index]) * (frame_count / boundary[-1])) + seg_frame
						img = all_frames[cap_frame_index]
						# print(video)
						# print(segment_index)
						# print(cap_frame_index)
						...
						bias = 0
						while len(crop_img) == 0:
							cap_frame_index = int((boundary[segment_index]) * (frame_count / boundary[-1])) + bias
							img = all_frames[cap_frame_index]
							# print(video)
							# print(segment_index)
							# print(cap_frame_index)
	...
	f = open('./feature/'+ SAVE_DATE + 'visual_seqs.pkl','wb')
	pickle.dump(visual_seqs,f)
	f.close()	
	f = open('./feature/' + SAVE_DATE +'seqs.pkl','wb')
	pickle.dump(seqs,f)
	f.close()	
	f = open('./feature/'+ SAVE_DATE + 'img_feature.pkl','wb')
	pickle.dump(img_feature,f)
	f.close()
	f = open('./feature/'+ SAVE_DATE + 'speech_emotion_seqs.pkl','wb')
